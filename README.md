...
- 输出 Clash/Mihomo YAML、base64 分享链接列表和 PAC 文件，并支持多格式下载。
- Clash/Mihomo 输出可引用聚合直连规则，避免国内站点默认走代理。
...
首次进入会要求设置管理 token。全局设置、用户和授权码会持久化在 `./data/config.json`；每个用户的订阅源、规则源、直连方案和公开订阅会分别保存在 `./data/users/{userID}.json`。admin 可以创建一次性授权码，用户注册后会拥有独立订阅源、公开订阅和备份空间。
...
同一个公开订阅会自动带 PAC 文件地址：

```text
/s/main.pac
/u/<USER_SLUG>/s/main.pac
```

如果设置了公开订阅用户 token，PAC 地址同样需要携带 `?token=<TOKEN>`。

PAC 和 Clash/Mihomo 的直连规则在“规则”页统一管理：

- 默认代理返回值为 `PROXY 127.0.0.1:7890; SOCKS5 127.0.0.1:7890; DIRECT`，与生成的 Clash/Mihomo `mixed-port` 保持一致。
- 规则源可单独管理，支持在线 URL、镜像内置文件和 `clash-domain`、`dnsmasq`、纯域名列表格式。
- 规则集可引用多个规则源，保存后可立即同步，系统会聚合去重并缓存到本地配置。
- 聚合规则可追加手工直连域名、排除不想直连的域名后缀、配置直连 CIDR，也可配置域名关键词生成 `DOMAIN-KEYWORD,<keyword>,节点选择`。
- 每个公开订阅在 PAC 规则里选择一个规则集，PAC 与 Clash/Mihomo YAML 会引用同一份聚合结果。
