...
func Render(output domain.Output, result Result, resolvedConfigs ...domain.PACConfig) ([]byte, string, error) {
	...
	case "surge":
		return []byte(renderSurge(output, result, resolvedPAC)), "text/plain; charset=utf-8", nil
	default:
...
func renderSurge(output domain.Output, result Result, resolvedPAC domain.PACConfig) string {
	lines := []string{
		"# Generated by Sub Nest for " + output.Slug,
		"",
		"[General]",
		"loglevel = notify",
		"",
	...
	lines = append(lines, surgeGroupLine("节点选择", "select", groupNames(append(result.Groups, result.SourceGroups...), true)))
	lines = append(lines, surgeGroupLine("自动选择", "url-test", allNodeNames(result.Nodes))+", url=http://www.gstatic.com/generate_204, interval=300")
	lines = append(lines, surgeGroupLine("故障切换", "fallback", allNodeNames(result.Nodes))+", url=http://www.gstatic.com/generate_204, interval=300")
	for _, group := range result.Groups {
		if strings.HasSuffix(group.Name, "节点") && len(group.Nodes) > 0 {
			lines = append(lines, surgeGroupLine(group.Name, "select", group.Nodes))
		}
	}
	for _, group := range result.SourceGroups {
		if len(group.Nodes) > 0 {
	...
	lines = append(lines, surgeDirectRules(resolvedPAC)...)
	return strings.Join(lines, "\n") + "\n"
}

func surgeProxyLine(node domain.Node) string {
	name := surgeToken(node.Name)
	server := surgeToken(node.Server)
	if name == "" || server == "" || node.Port == 0 {
		return ""
	}
	parts := []string{name + " = " + surgeProxyType(node.Type), server, fmt.Sprintf("%d", node.Port)}
	for _, field := range surgeProxyFields(node) {
		parts = append(parts, field)
	}
	return strings.Join(parts, ", ")
}

func surgeProxyType(typ string) string {
...
func surgeProxyFields(node domain.Node) []string {
	fields := []string{}
	typ := strings.ToLower(strings.TrimSpace(node.Type))
	switch typ {
	case "ss":
		if cipher := extraString(node.Extra, "cipher"); cipher != "" {
			fields = append(fields, "encrypt-method="+surgeValue(cipher))
		}
		if password := extraString(node.Extra, "password"); password != "" {
			fields = append(fields, "password="+surgeValue(password))
		}
	case "vmess":
		if uuid := firstExtraString(node.Extra, "uuid", "id", "user"); uuid != "" {
			fields = append(fields, "username="+surgeValue(uuid))
		}
		if cipher := extraString(node.Extra, "scy"); cipher != "" && cipher != "auto" {
			fields = append(fields, "encrypt-method="+surgeValue(cipher))
		}
	case "trojan":
		if password := firstExtraString(node.Extra, "password", "user"); password != "" {
			fields = append(fields, "password="+surgeValue(password))
		}
	}
	if sni := firstExtraString(node.Extra, "sni", "peer", "host"); sni != "" {
		fields = append(fields, "sni="+surgeValue(sni))
	}
	if tlsValue := firstExtraString(node.Extra, "tls", "security"); tlsValue == "tls" || tlsValue == "1" || tlsValue == "true" {
		fields = append(fields, "tls=true")
	}
	if network := firstExtraString(node.Extra, "network", "net"); network == "ws" {
		fields = append(fields, "ws=true")
		if path := firstExtraString(node.Extra, "path"); path != "" {
			fields = append(fields, "ws-path="+surgeValue(path))
		}
		if host := firstExtraString(node.Extra, "host"); host != "" {
			fields = append(fields, "ws-headers=Host:"+surgeValue(host))
		}
	}
	return fields
}

func surgeGroupLine(name string, typ string, proxies []string) string {
	parts := []string{surgeToken(name) + " = " + typ}
	for _, proxy := range proxies {
		if token := surgeToken(proxy); token != "" {
...
func surgeDirectRules(cfg domain.PACConfig) []string {
	cfg = domain.NormalizePACConfig(cfg)
	rules := []string{}
	for _, keyword := range cfg.DomainKeywords {
		rules = append(rules, "DOMAIN-KEYWORD,"+surgeToken(keyword)+",节点选择")
	}
	for _, suffix := range pacDomainSuffixes(cfg) {
		if suffix == "local" {
			continue
		}
		rules = append(rules, "DOMAIN-SUFFIX,"+surgeToken(suffix)+",DIRECT")
	}
	for _, cidr := range cfg.DirectCIDRs {
		rules = append(rules, "IP-CIDR,"+surgeToken(cidr)+",DIRECT,no-resolve")
	}
	rules = append(rules, "GEOIP,CN,DIRECT", "FINAL,节点选择")
	return rules
}

func surgeToken(value string) string {
...
func surgeValue(value string) string {
	return surgeToken(value)
}
